...
mod tests {
    ...
    fn prefix2_assignments() {
        ...
        loop {
            ...
            assert(ImplLoot::get_prefix2(ItemId::Warhammer, i) == ItemNameSuffix::Bane, 'warhammer should be bane');

            // Quarterstaffs are always 'X Root'
            assert(
                ImplLoot::get_prefix2(ItemId::Quarterstaff, i) == ItemNameSuffix::Root, 'quarterstaff should be root',
            );

            // Mauls are always 'X Bite'
            assert(ImplLoot::get_prefix2(ItemId::Maul, i) == ItemNameSuffix::Bite, 'maul should be bite');

            // Maces are always 'X Song'
            assert(ImplLoot::get_prefix2(ItemId::Mace, i) == ItemNameSuffix::Song, 'mace should be song');

            // Clubs are always 'X Roar'
            assert(ImplLoot::get_prefix2(ItemId::Club, i) == ItemNameSuffix::Roar, 'club should be roar');

            // Katanas are always 'X Grasp'
            assert(ImplLoot::get_prefix2(ItemId::Katana, i) == ItemNameSuffix::Grasp, 'katana should be grasp');
            ...
            assert(ImplLoot::get_prefix2(ItemId::Scimitar, i) == ItemNameSuffix::Glow, 'scimitar should be glow');

            // Long Swords are always 'X Bender'
            assert(
                ImplLoot::get_prefix2(ItemId::LongSword, i) == ItemNameSuffix::Bender, 'long sword should be bender',
            );

            // Short Swords are always 'X Shadow'
            assert(
                ImplLoot::get_prefix2(ItemId::ShortSword, i) == ItemNameSuffix::Shadow, 'short sword should be shadow',
            );

            // Ghost Wands are always 'X Whisper'
            assert(
                ImplLoot::get_prefix2(ItemId::GhostWand, i) == ItemNameSuffix::Whisper, 'ghost wand should be whisper',
            );

            // Grave Wands are always 'X Shout'
            assert(ImplLoot::get_prefix2(ItemId::GraveWand, i) == ItemNameSuffix::Shout, 'grave wand should be shout');

            // Bone Wands are always 'X Growl'
            assert(ImplLoot::get_prefix2(ItemId::BoneWand, i) == ItemNameSuffix::Growl, 'bone wand should be growl');

            // Wands are always 'X Tear'
            assert(ImplLoot::get_prefix2(ItemId::Wand, i) == ItemNameSuffix::Tear, 'wand should be tear');

            // Grimoires are always 'X Peak'
            assert(ImplLoot::get_prefix2(ItemId::Grimoire, i) == ItemNameSuffix::Peak, 'grimoire should be peak');

            // Chronicles are always 'X Form'
            assert(ImplLoot::get_prefix2(ItemId::Chronicle, i) == ItemNameSuffix::Form, 'chronicle should be form');

            // Tomes are always 'X Sun'
            assert(ImplLoot::get_prefix2(ItemId::Tome, i) == ItemNameSuffix::Sun, 'tome should be sun');

            // Books are always 'X Moon'
            assert(ImplLoot::get_prefix2(ItemId::Book, i) == ItemNameSuffix::Moon, 'book should be moon');

            // Chest Armor
            //
            // Divine Robes are always {X Bane, X Song, X Instrument, X Shadow, X Growl, X Form}
            // (set 1)
            assert(is_special3_set1(ImplLoot::get_prefix2(ItemId::DivineRobe, i)), 'invalid divine robe name suffix');

            // Chain Mail is always {X Root, X Roar, X Glow, X Whisper, X Tear, X Sun} (set 2)
            assert(is_special3_set2(ImplLoot::get_prefix2(ItemId::ChainMail, i)), 'invalid chain mail name suffix');

            // Demon Husks are always {X Bite, X Grasp, X Bender, X Shout, X Peak, X Moon} (set 3)
            assert(is_special3_set3(ImplLoot::get_prefix2(ItemId::DemonHusk, i)), 'invalid demon husk name suffix');
            //

            // Head Armor
            //
            // Ancient Helms use name suffix set 1
            assert(is_special3_set1(ImplLoot::get_prefix2(ItemId::AncientHelm, i)), 'invalid war cap name suffix');

            // Crown uses name suffix set 2
            assert(is_special3_set2(ImplLoot::get_prefix2(ItemId::Crown, i)), 'invalid crown name suffix');

            // Divine Hood uses name suffix set 3
            assert(is_special3_set3(ImplLoot::get_prefix2(ItemId::DivineHood, i)), 'invalid divine hood name suffix');

            //
            // Waist Armor
            //
            // Ornate Belt uses name suffix set 1
            assert(is_special3_set1(ImplLoot::get_prefix2(ItemId::OrnateBelt, i)), 'invalid ornate belt suffix');

            // Brightsilk Sash uses name suffix set 2
            assert(
                is_special3_set2(ImplLoot::get_prefix2(ItemId::BrightsilkSash, i)), 'invalid brightsilk sash suffix',
            );

            // Hard Leather Belt uses name set 3
            assert(
                is_special3_set3(ImplLoot::get_prefix2(ItemId::HardLeatherBelt, i)), 'wrong hard leather belt suffix',
            );
            ...
            assert(is_special3_set1(ImplLoot::get_prefix2(ItemId::HolyGreaves, i)), 'invalid holy greaves suffix');

            // Heavy Boots use name suffix set 2
            assert(is_special3_set2(ImplLoot::get_prefix2(ItemId::HeavyBoots, i)), 'invalid heavy boots suffix');

            // Silk Slippers use name suffix set 3
            assert(is_special3_set3(ImplLoot::get_prefix2(ItemId::SilkSlippers, i)), 'invalid silk slippers suffix');

            //
            // Hand Armor
            //
            // Holy Gauntlets use name suffix set 1
            assert(is_special3_set1(ImplLoot::get_prefix2(ItemId::HolyGauntlets, i)), 'invalid holy gauntlets suffix');

            // Linen Gloves use name suffix set 2
            assert(is_special3_set2(ImplLoot::get_prefix2(ItemId::LinenGloves, i)), 'invalid linen gloves suffix');
            ...
            assert(is_special3_set1(ImplLoot::get_prefix2(ItemId::Necklace, i)), 'invalid Necklace name suffix');

            // Amulets use name suffix set 2
            assert(is_special3_set2(ImplLoot::get_prefix2(ItemId::Amulet, i)), 'invalid amulet name suffix');

            // Pendants use name suffix set 3
            assert(is_special3_set3(ImplLoot::get_prefix2(ItemId::Pendant, i)), 'invalid pendant name suffix');
    ...
    fn prefix1_assignment() {
        ...
        loop {
            ...
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::Warhammer, i)), 'invalid warhammer prefix');

            // verify quarterstaff uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::Quarterstaff, i)), 'invalid quarterstaff prefix');
            ...
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::Katana, i)), 'invalid katana prefix');

            // verify falchion uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::Falchion, i)), 'invalid falchion prefix');

            // verify scimitar uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::Scimitar, i)), 'invalid scimitar prefix');

            // verify long sword uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::LongSword, i)), 'invalid long sword prefix');

            // verify short sword uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::ShortSword, i)), 'invalid short sword prefix');

            // verify ghost wand uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::GhostWand, i)), 'invalid ghost wand prefix');

            // verify grave wand uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::GraveWand, i)), 'invalid grave wand prefix');

            // verify bone wand uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::BoneWand, i)), 'invalid bone wand prefix');
            ...
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::Grimoire, i)), 'invalid grimoire prefix');

            // verify chronicle uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::Chronicle, i)), 'invalid chronicle prefix');
            ...
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::DivineRobe, i)), 'invalid divine robe prefix');

            // verify silk robe uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::SilkRobe, i)), 'invalid silk robe prefix');

            // verify linen robe uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::LinenRobe, i)), 'invalid linen robe prefix');
            ...
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::Shirt, i)), 'invalid shirt prefix');

            // verify demon husk uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::DemonHusk, i)), 'invalid demon husk prefix');

            // verify dragonskin armor uses set1
            assert(
                is_special2_set1(ImplLoot::get_prefix1(ItemId::DragonskinArmor, i)), 'invalid dragonskin armor prefix',
            );
            ...
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::HardLeatherArmor, i)), 'invalid hard leather prefix');

            // verify leather armor uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::LeatherArmor, i)), 'invalid leather armor prefix');

            // verify holy chestplate uses set2
            assert(
                is_special2_set2(ImplLoot::get_prefix1(ItemId::HolyChestplate, i)), 'invalid holy chestplate prefix',
            );

            // verify ornate chestplate uses set3
            assert(
                is_special2_set3(ImplLoot::get_prefix1(ItemId::OrnateChestplate, i)), 'invalid ornte chestplate prefix',
            );

            // verify plate mail uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::PlateMail, i)), 'invalid plate mail prefix');

            // verify chain mail uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::ChainMail, i)), 'invalid chain mail prefix');

            // verify ring mail uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::RingMail, i)), 'invalid ring mail prefix');

            // assert ancient helm uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::AncientHelm, i)), 'invalid ancient helm prefix');

            // assert ornate helm uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::OrnateHelm, i)), 'invalid ornate helm prefix');

            // assert great helm uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::GreatHelm, i)), 'invalid great helm prefix');

            // assert full helm uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::FullHelm, i)), 'invalid full helm prefix');
            ...
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::DemonCrown, i)), 'invalid demon crown prefix');

            // assert dragons crown uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::DragonsCrown, i)), 'invalid dragons crown prefix');

            // assert war cap uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::WarCap, i)), 'invalid war cap prefix');

            // assert leather cap uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::LeatherCap, i)), 'invalid leather cap prefix');

            // assert cap uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::Cap, i)), 'invalid cap prefix');

            // assert crown uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::Crown, i)), 'invalid crown prefix');

            // assert divine hood uses set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::DivineHood, i)), 'invalid divine hood prefix');

            // assert silk hood uses set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::SilkHood, i)), 'invalid silk hood prefix');

            // assert linen hood uses set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::LinenHood, i)), 'invalid linen hood prefix');
            ...
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::OrnateBelt, i)), 'invalid ornate belt prefix');

            // assert war belt is set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::WarBelt, i)), 'invalid war belt prefix');

            // assert plated belt is set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::PlatedBelt, i)), 'invalid plated belt prefix');

            // assert mesh belt is set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::MeshBelt, i)), 'invalid mesh belt prefix');

            // assert heavy belt is set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::HeavyBelt, i)), 'invalid heavy belt prefix');

            // assert demonhide belt is set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::DemonhideBelt, i)), 'invalid demonhide belt prefix');

            // assert dragonskin belt is set1
            assert(
                is_special2_set1(ImplLoot::get_prefix1(ItemId::DragonskinBelt, i)), 'invalid dragonskin belt prefix',
            );
            ...
            assert(
                is_special2_set3(ImplLoot::get_prefix1(ItemId::HardLeatherBelt, i)), 'invalid hard leather blt prefix',
            );

            // assert leather belt is set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::LeatherBelt, i)), 'invalid leather belt prefix');

            // assert brightsilk sash is set2
            assert(
                is_special2_set2(ImplLoot::get_prefix1(ItemId::BrightsilkSash, i)), 'invalid brightsilk sash prefix',
            );

            // assert silk sash is set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::SilkSash, i)), 'invalid silk sash prefix');

            // assert wool sash is set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::WoolSash, i)), 'invalid wool sash prefix');

            // assert linen sash is set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::LinenSash, i)), 'invalid linen sash prefix');

            // assert sash is set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::Sash, i)), 'invalid sash prefix');

            // assert holy greaves is set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::HolyGreaves, i)), 'invalid holy greaves prefix');

            // assert ornate greaves is set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::OrnateGreaves, i)), 'invalid ornate greaves prefix');

            // assert greaves is set3
            assert(is_special2_set3(ImplLoot::get_prefix1(ItemId::Greaves, i)), 'invalid greaves prefix');

            // assert chain boots is set1
            assert(is_special2_set1(ImplLoot::get_prefix1(ItemId::ChainBoots, i)), 'invalid chain boots prefix');

            // assert heavy boots is set2
            assert(is_special2_set2(ImplLoot::get_prefix1(ItemId::HeavyBoots, i)), 'invalid heavy boots prefix');

            // assert demonhide boots is set3
            assert(
                is_special2_set3(ImplLoot::get_prefix1(ItemId::DemonhideBoots, i)), 'invalid demonhide boots prefix',
            );

            // assert dragonskin boots is set1
            assert(
                is_special2_set1(ImplLoot::get_prefix1(ItemId::DragonskinBoots, i)), 'invalid dragonskin boots prefix',
            );
    ...
    fn get_item_part3() {
        ...
        assert(studded_leather_armor_item.item_type == Type::Blade_or_Hide(()), 'studded leather armor is hide');
        ...
        assert(hard_leather_armor_item.item_type == Type::Blade_or_Hide(()), 'hard leather armor is hide');
        ...
        assert(studded_leather_belt_item.item_type == Type::Blade_or_Hide(()), 'studded leather belt is hide');
        ...
        assert(hard_leather_belt_item.item_type == Type::Blade_or_Hide(()), 'hard leather belt is hide');
    ...
    fn get_item_part4() {
        ...
        assert(studded_leather_boots_item.item_type == Type::Blade_or_Hide(()), 'studded leather boots is hide');
        ...
        assert(hard_leather_boots_item.item_type == Type::Blade_or_Hide(()), 'hard leather boots is hide');
        ...
        assert(dragonskin_gloves_item.item_type == Type::Blade_or_Hide(()), 'dragonskin gloves is hide');
        ...
        assert(studded_leather_gloves_item.item_type == Type::Blade_or_Hide(()), 'studded leather gloves is hide');
        ...
        assert(hard_leather_gloves_item.item_type == Type::Blade_or_Hide(()), 'hard leather gloves is hide');
    ...
    fn get_type() {
        ...
        assert(studded_leather_gloves_type == Type::Blade_or_Hide(()), 'studded leather gloves is hide');
